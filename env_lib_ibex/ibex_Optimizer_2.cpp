...
Optimizer::Status Optimizer::optimize() {
    ...
    const std::string original_problem_file = "/home/abel/Documents/ANN-HH-NCOP-Solver/tests/batch1/ex6_2_11.bch";
    ...
    // Copiar el archivo original al archivo de trabajo
    ...
    try {
		int contador = 0;
		std::string bisector;
		std::string bisectorPorDefecto;
		bisectorPorDefecto = "LSmear";
        while (!buffer.empty()) {
            double prec = 1e-9;
			...
			if ((thebuffer->currentbuffer.empty() && thebuffer->futurebuffer.empty()) || contador == 0) {
				...
				//std::cout << "Respuesta de ANN: " << result << std::endl;
				...
				bisector = bisectorPorDefecto;
			}
            
			if (trace >= 2) std::cout << " current box " << c->box << std::endl;

			// Procesar la celda como de costumbre
			try {
				if (bisector == "LSmear") {
					//std::cout << "C++: LSmear" << std::endl;
					...
					//std::cout << "C++: RoundRobin" << std::endl;
					...
					//std::cout << "C++: LargestFirst" << std::endl;
